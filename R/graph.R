...
graph.execute <- function (ctx, datasetName, analysisName, params) {
    ...
    for (idx in 1:length(clusterIdentityLevels)) {
        ...
        utils::write.table(nodeData, file=nodeDataFile, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
    }
}
#
...
graph.executePlots <- function (ctx, datasetName, analysisName, plotList, params) {
    ...
    for (idx in 1:length(clusterIdentityLevels)) {
        ...
        nodeData <- utils::read.table(nodeDataFile, as.is=TRUE, header=TRUE, sep="\t")
        rownames(nodeData) <- nodeData$Node
	...
	# Perform plots for this onnectivity threshold
        ...
        graph.makeClusterPlot (ctx, gr, nodeData, analysisName, thresholdValue, layout, plotFilenameRoot)            
    }
}
#
...
graph.getEdgeWeightColours <- function(weights) {
    ...
    grays <- grDevices::gray.colors(steps, start=0.0, end=0.95)		#; grays <- rainbow(steps)
