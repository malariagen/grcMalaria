...
################################################################################
#
pca.execute <- function(analysisContext, analysisName, pcaMethod) {
    dataFolder <- getOutFolder(analysisName, c(pcaMethod, "data"))
    sampleMeta <- analysisContext$meta
    distData  <- analysisContext$distance
    genosData <- analysisContext$genos
    ...
    if (pcaMethod == "umap") {
        ...
        pcaResults <- NULL
        if (pcaMethod == "bpca") {
            pcaResults <- pca(genos, method=pcaMethod, center=TRUE, scale="uv", nPcs=phylo.pcCount, maxSteps=500)
        } else {
            pcaResults <- pca(genos, method=pcaMethod, center=TRUE, scale="uv", nPcs=phylo.pcCount)
        }
        pcScores <- pcaResults@scores
        varExplained <- pcaResults@R2
    }
...
pca.executePlots <- function(analysisContext, analysisName, pcaMethod, plotList) {
    dataFolder <- getOutFolder(analysisName, c(pcaMethod, "data"))
    plotsRootFolder <- getOutFolder(analysisName, c(pcaMethod, "plots"))
    ...
    for (plotIdx in 1:length(plotList)) {
        ...
        #pca.plotTwoComponents (plotName, plotMetadata, legendData, "PC1", "PC2", plotsFolder);
